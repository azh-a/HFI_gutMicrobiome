...
## Libraries
```{r libraries}
# be careful in regards to package functions overlapping
library(phyloseq)
library(vegan)
library(rstatix)
library(Maaslin2)
...
## Load Data
```{r data}
ps_gutMicro <- readRDS("zhu_final_ps.rds")
...
beta_diversity_bray_plot_categorical <- function(ps, variable, title_name=variable, seed = 5) {
  ...
  # only get values that appear at least 3 times (PCOA plot can't draw ellipse with < 3 points)
...
## ML (achieved 84% balanced accuracy)
```{r ML}
...
run_picrust_daa_metacyc <- function(
  ...
  # Check if adjusted p's are significant
  ...
  if (use_raw_p_for_plot) {
    ...
    daa_for_plot <- annot
    daa_for_plot[[p_adj_col]] <- annot[[p_col]]   
